...
export default function CatalogPage() {
  ...
  useEffect(() => {
    ...
    if (debouncedQuery) analyticsService.track("search", { query: debouncedQuery });
  ...
  const filtered = useMemo(() => {
    ...
    const result = products.filter((product) => {
      ...
      return (
        ...
        fromMinor(activePromotion?.specialPrice ?? product.salePrice) <= maxPrice
      );
    });
    return result.sort((a, b) => {
      ...
      if (sort === "discount") return calculateDiscountPercent(priceB, b.oldPrice) - calculateDiscountPercent(priceA, a.oldPrice);
      return b.views - a.views;
    });
  }, [products, debouncedQuery, category, brand, promotion, maxPrice, sort]);
    ...
    <div className="catalog-filters market-filter-sheet">
      ...
      <div className="filter-group">
        ...
        <div className="filter-options">
          <label><input type="radio" checked={category === "all"} onChange={() => setCategory("all")} /><span>{t("allProducts")}</span></label>
          {categories.filter((item) => item.isVisible).map((item) => (
            <label key={item.id}>
              <input type="radio" checked={category === item.id} onChange={() => {
                setCategory(item.id);
      ...
      <div className="filter-group">
        ...
        <div className="promotion-filter-grid">
          {[
            ...
            <button key={value} className={promotion === value ? "active" : ""} onClick={() => setPromotion(value)}>{label}</button>
          ))}
        </div>
      </div>
      <Button block className="mobile-filter-apply" onClick={() => setFilterOpen(false)}>
      ...
      <div className="catalog-layout market-catalog-layout">
        ...
        <section className="catalog-results">
          <div className="catalog-result-meta">
            <span>{t("found")}: <strong>{filtered.length}</strong></span>
            {activeFilterCount > 0 && <button onClick={reset}>{t("reset")} <X size={14} /></button>}
          </div>
          {loading ? <ProductGrid products={[]} skeleton /> : visible.length ? <ProductGrid products={visible} /> : (
            <EmptyState icon={<Search size={30} />} title={t("nothingFound")} text={t("nothingFoundText")} action={<Button onClick={reset}>{t("reset")}</Button>} />
          )}
          {totalPages > 1 && (
            <nav className="pagination" aria-label={t("catalogPages")}>
              <button onClick={() => setPage((value) => Math.max(1, value - 1))} disabled={page === 1} aria-label={t("previousPage")}><ChevronLeft size={18} /></button>
              {Array.from({ length: totalPages }, (_, index) => index + 1).map((number) => (
                <button key={number} className={page === number ? "active" : ""} onClick={() => setPage(number)}>{number}</button>
              ))}
              <button onClick={() => setPage((value) => Math.min(totalPages, value + 1))} disabled={page === totalPages} aria-label={t("nextPage")}><ChevronRight size={18} /></button>
            </nav>
          )}
        </section>
      </div>
      <Modal open={filterOpen} onClose={() => setFilterOpen(false)} title={t("filters")} size="full"><FilterContent /></Modal>
    </div>
  );
}
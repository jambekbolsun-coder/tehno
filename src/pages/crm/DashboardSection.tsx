...
export function DashboardSection({ role }: { role: "admin" | "manager" }) {
  ...
  return (
    <div className="crm-page dashboard-section">
      ...
      {role === "admin" ? <div className="metrics-grid"><MetricCard label="Новые заявки" value={String(newLeads)} helper={`${leads.filter((lead) => lead.status === "working").length} уже в работе`} trend={12} icon={ClipboardList} accent="blue"/><MetricCard label="Продажи за месяц" value={formatMoney(summary.revenue)} helper={`${recognizedOrders.length} завершённых продаж`} trend={8} icon={ShoppingCart} accent="green"/><MetricCard label="Чистая прибыль" value={formatMoney(summary.accrualProfit)} helper="По завершённым заказам" trend={5} icon={TrendingUp} accent="violet"/><MetricCard label="Расходы" value={formatMoney(summary.expenses)} helper="Без двойного учёта" trend={-3} icon={ReceiptText} accent="orange"/><MetricCard label="На складе" value={`${products.reduce((sum, product) => sum + product.stock, 0)} шт.`} helper={`${lowStock.length} позиций требуют внимания`} icon={Boxes} accent="cyan"/><MetricCard label="Долги поставщикам" value={formatMoney(summary.supplierDebt)} helper="За завершённые продажи" icon={Wallet} accent="red"/></div> : <div className="metrics-grid metrics-grid--manager"><MetricCard label="Мои новые заявки" value={String(newLeads)} helper={`${leads.length} всего назначено`} icon={ClipboardList} accent="blue"/><MetricCard label="Мои продажи" value={formatMoney(revenue)} helper={`${recognizedOrders.length} завершённых продаж`} trend={9} icon={ShoppingCart} accent="green"/><MetricCard label="Конверсия" value={`${conversion}%`} helper={`${completed} завершённых заявок`} icon={TrendingUp} accent="violet"/><MetricCard label="Мои начисления" value={formatMoney(commission)} helper="До учёта выплат" icon={BadgeDollarSign} accent="orange"/></div>}

      <div className="dashboard-main-grid">
        <section className="crm-panel sales-chart-panel"><header><div><span>Продажи</span><h3>Динамика за неделю</h3></div><b>+8,4%</b></header><BarChart data={salesData} color="#4f7cff"/><footer><div><span className="legend-dot legend-dot--blue"/>Завершено</div><strong>{recognizedOrders.length} заказов</strong></footer></section>

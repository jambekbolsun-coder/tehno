...
const promoKey: Record<
  PromotionType,
  "sale" | "promoDiscount" | "cashback" | "giveaway" | "hit" | "new"
> = {
  sale: "sale",
  ...
  return (
    <article className="product-card">
      <div className="product-card__media">
        {activePromotion && (
          <span
            className={`promo-ribbon promo-ribbon--${activePromotion.type}`}
          >
      ...
      <div className="product-card__body">
        ...
        {activePromotion?.endAt && (
          <span className="product-card__deadline">
            {t("until")} {formatDate(activePromotion.endAt)}
          </span>
        )}
        <div className="product-card__footer">
          <span className={`stock-dot${available > 0 ? "" : " is-empty"}`}>
            {available > 0 ? `${t("inStock")} · ${available}` : t("outOfStock")}
          </span>
          <button
